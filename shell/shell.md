...
Entre o emulador e o shell existe um **canal de texto** (TTY/PTY). Detalhes em [Shell: TTY, PTY e sessões](sessoes.md).
...
6. **Empilhar shells sem perceber** — `zsh` → `bash` → script: variáveis e `cd` do nível interno não voltam ao pai ao dar `exit` (comportamento esperado, mas confunde no início). Ver [empilhamento e TTY](sessoes.md).
...
4. ➡️ Aprofundamento: [TTY/PTY, empilhamento, Windows e macOS](sessoes.md).
